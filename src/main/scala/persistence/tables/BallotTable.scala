...
class BallotTable(val profile: JdbcProfile) extends TableUtils {
  ...
  def createTableIfNotExists(db: JdbcProfile#Backend#Database)(implicit ec: ExecutionContext): Future[Unit] = {
    for {
      ...
      _ <- createIndexIfNotExists(db, "idx_lottery_user", "CREATE INDEX idx_lottery_user ON ballots (lottery_id, user_id)")
      _ <- createIndexIfNotExists(db, "idx_lottery_id", "CREATE INDEX idx_lottery_id ON ballots (lottery_id)")
    } yield ()
  }
}
